...
export default function LoginPage() {
    ...
    return (
            ...
            <div className="max-w-md w-full glass-panel !bg-white/90 !rounded-[3.5rem] p-10 md:p-12 relative z-10 animate-in zoom-in-95 duration-500 shadow-2xl">
                <div className="flex flex-col items-center mb-10">
                    <div className="w-20 h-20 bg-gradient-to-br from-slate-900 to-slate-800 rounded-[2rem] flex items-center justify-center shadow-xl shadow-slate-900/20 mb-6 transform -rotate-3 hover:rotate-0 transition-transform duration-500">
                        <svg className="w-10 h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M13 10V3L4 14h7v7l9-11h-7z" />
                        </svg>
                    </div>
                    <h1 className="text-4xl font-black text-slate-900 tracking-tight leading-tight italic">INTERVOUS</h1>
                    <p className="text-slate-500 mt-2 text-center font-bold text-base">Continue your professional adventure.</p>
                </div>

                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-3">
                        <label className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] ml-3">Access Email</label>
                        <input
                            type="email"
                    ...
                    <div className="space-y-3">
                        <label className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] ml-3">Password</label>
                        <input
                            type="password"
                            placeholder="••••••••••••"
                            value={password}
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full py-5 px-6 font-black rounded-3xl text-xs uppercase tracking-[0.2em] text-white bg-slate-900 hover:bg-slate-800 shadow-xl shadow-slate-900/20 hover:shadow-2xl hover:shadow-slate-900/30 transition-all transform hover:-translate-y-1 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center gap-3 overflow-hidden group mt-4 ring-1 ring-white/20"
                    >
                        {loading ? (
                            <>
                    ...
                    {errorMsg && (
                        <div className="bg-rose-50 border-2 border-rose-100 text-rose-600 text-[11px] font-black py-4 px-6 rounded-3xl flex items-center gap-3 animate-in shake-in">
